...
impl Viewport {
    pub fn fit_to_audio(&mut self, audio: &AudioBuffer, width: f32) {
        ...
        self.samples_per_pixel = (audio.total_samples().max(1) as f32 / width)
    ...
    pub fn visible_frame_range(&self, audio: &AudioBuffer, width: f32) -> Range<usize> {
        ...
        let end_frame = ((end_sample + channels.saturating_sub(1)) / channels).min(audio.frame_count());
    ...
    pub fn zoom_around(
        ...
        let width = width.max(1.0);
        let old_spp = self.samples_per_pixel.max(audio.channels_usize() as f32 / 32.0);
    ...
    pub fn pan_by_fraction(&mut self, audio: &AudioBuffer, width: f32, fraction: f32) {
        ...
        self.pan_samples(audio, width, (visible_samples as f32 * fraction).round() as isize);
    }
